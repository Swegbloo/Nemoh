...
CONTAINS
  ...
  SUBROUTINE VAV(KKK,H,ZER)
    ...
    REAL:: H,ZER,DH,XOI,YOI,ZOI,XGI,YGI,ZGI,DLOGV
	...
	DO J=1,IMX
		...
		ELSE
			...
			IF(DK.GE.1.E-3*TDIS(J))THEN                                             
			    ...
			    DLOGV=(DNL-ANL)/DENL                                                       
			    VXS(JJ)=VXS(JJ)+GYX*ALDEN+GY*ANLX*DLOGV-2.*ABS(GZ)*ATX-DAT*A3J
			    VYS(JJ)=VYS(JJ)+GYY*ALDEN+GY*ANLY*DLOGV-2.*ABS(GZ)*ATY-DAT*A6J             
			    VZS(JJ)=VZS(JJ)+GYZ*ALDEN+GY*ANLZ*DLOGV-2.*ABS(GZ)*ATZ-DAT*A9J            
			ENDIF
		    END DO
  ...
  SUBROUTINE VNS(T,BETA,VA,ZER)
    ...
    COMPLEX*8 ZI,C1,C2,ASD,BSD,CSD,ZA0,ZB,ZVS
