...
C  (P,Q,R): NORMALES DES FACETTES ET DES FACETTES DE LA FLOTTAISON
     ...
     #RHO,TPER,BETA1,BETA2
      ...
      COMPLEX ZA1(6,LN),ZA2(6,LN),ZAX(6,LN),ZET,ZZZ,ZF2
...
C *** LECTURE DE LA GEOMETRIE ***
     ...
     #((AMOR(I,J),I=1,JQ),J=1,JQ)
      ...
      DO I=1,NFFL
            ...
            R(IMX+I)=R(IND(I))            ! NORMALE A LA FLOTTAISON (NZ=0 --> FACETTE VERTICALE)
            ZM(IMX+I)=0.
            DO J=1,4
...
c~             ELSE  ! FACETTE HORIZONTALE A LA FLOTTAISON
c~                   CN(I,1)=P(I)/SQRT(P(I)**2+Q(I)**2)
c~                   CN(I,2)=Q(I)/SQRT(P(I)**2+Q(I)**2)
c~                   CN(I,3)=0.
c~                   CN(I,4)=(YM(I)-YEFF)*CN(I,3)-(ZM(I)-ZEFF)*CN(I,2)
c~                   CN(I,5)=(ZM(I)-ZEFF)*CN(I,1)-(XM(I)-XEFF)*CN(I,3)
c~                   CN(I,6)=(XM(I)-XEFF)*CN(I,2)-(YM(I)-YEFF)*CN(I,1)
c~             ENDIF
      END DO
...
c~       CALL FORINE(AINTINERTIEM,A1,FINT2)
      ...
      CALL FORINE(AINTR1,A1,FINT11)
      CALL FORINE(AINTM1,A1,FINT21)
...
!LONGUEUR DE LA LIGNE DE FLOTTAISON --> OUI! cf QTFGeom l.99
                  ...
                  DO J=1,6
                        COST=SQRT(1-CN(II,3)**2)
                        ACQ=CTB*GAMMA0*CN(II,J)*((-1.)**(J+1))**(JJ+1)
     1 /COST      ! attention cos(theta)
...
!! FONCTIONNE, MULTIPLICATION PAR i ETRANGE, ON DOIT FAIRE LA MEME AU EFFORTS D'INERTIE (3EME APPROX)
     ...
     1 /(WWX(IPM)-WWX(IPM-1))*(ZZX(J,KNC,IPM)-ZZX(J,KNC,IPM-1))
      ...
      DO IJ=1,6
      DO IK=1,6
            AINTV(IJ,IK)=AIN(IJ,IK,KNC)
      END DO
      END DO
      CALL FORINE(AINTV,A1,FINT1)
      DIMM=WM*WM
