...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !!!!!!!!!!!!!                                           !!!!!!!!!!!!!!!!!
      OPEN(10,FILE=ID(1:lID)//'/mesh/SF_L12.dat',STATUS='OLD')
      DO IOP=1,7
        READ(10,*)
      ENDDO
      READ(10,*) NSYMY,NFACSL,NCONT,NOMBPO,NLIGNE,RCEREXT,NPASR,NTOT
      WRITE(*,*) "NSYMY =",NSYMY,"NFACSL =",NFACSL,"NCONT =",NCONT,
     1 "NOMBPO =",NOMBPO,"NLIGNE =",NLIGNE,"RCEREXT =",RCEREXT,
     1 "NPASR =",NPASR,"NTOT =",NTOT
      DO IOP=1,NOMBPO
        READ(10,*) XSL(IOP),YSL(IOP)
      ENDDO
c~       WRITE(*,*) 'XSL,YSL'
      DO IOP=1,NFACSL+NCONT+NLIGNE
        READ(10,*) XMSL(IOP),YMSL(IOP)
      ENDDO
c~       WRITE(*,*) 'XMSL,YMSL'
      DO IOP=1,NFACSL+NCONT
        READ(10,*) AIRESL(IOP)
      ENDDO
c~       WRITE(*,*) 'AIRESL'
      DO IOP=1+NFACSL,NCONT+NFACSL
        READ(10,*) CNSL(1,IOP),CNSL(2,IOP)
      ENDDO
c~       WRITE(*,*) 'CNSL'
      DO IOP=1,NFACSL+NCONT
        READ(10,*) M1SL(IOP),M2SL(IOP)
      ENDDO
c~       WRITE(*,*) 'M1SL,M2SL'
      DO IOP=1,NFACSL
        READ(10,*) M3SL(IOP),M4SL(IOP)
      ENDDO
c~       WRITE(*,*) 'M3SL,M4SL'
      CLOSE(10) 
