...
C      
      COMMON CNC,NCO,NSYMY,NP,IMX,NFFL,IXX,XEFF,YEFF,ZEFF,RHO,TPER,BETA1,BETA2
      ...
      INTEGER REE,FICH2,DOF,IUNI,NQTFP,LQTFP,LHASK,OUT1,DOFSYM
...
C *** READING THE GEOMETRY ***
     ...
     #((AMOR(I,J),I=1,JQ),J=1,JQ)
      ...
      DO I=1,NFFL
            ...
            ZM(IMX+I)=0.                  ! NORMAL TO FLOTATION (NZ = 0 -> VERTICAL FACET)
            DO J=1,4
...
CFV NCO AU LIEU DE NC!!??
CFV NCO INSTEAD OF NC !! ??     
...
c~       DO I=1,NHASKIND    ! initialisation des sorties             ! version matrice carre pour les QTF+
      ...
      DO I=1,N   !N Number of freq ! initialisation des sorties      ! version matrice antitriangulaire sup pour les QTF+ (0 ailleurs)
            READ(21,REC=NT(I))TRB1,BETAB1,AMB1 !Encounter period, beta,wave number
            FH(I)=2*PI/TRB1                     ! omega [rad/s]
...
C ITERATION ON THE NUMBER OF INDEPENDENT DESCRIBED BODIES
C -------------------------------------------------------------
      DO KNC=1,NCO
      IF(NC.GT.1) THEN
...
C IA = 1 TO SEE !! IA = IMXC (KNC-1) +1                   
                  IA=1
C EST IL POSSIBLE QUE IPOS(KNC)>1, NON !
C IT IS POSSIBLE THAT IPOS (KNC)> 1, NO!
...
C -------------------------------------------------
      ...
      DO II=IA,IB
            ...
            END DO
            END DO
            CALL PRODT(PRO1,PRO2,FP,FM,3) !CALCUL NOUS DONNE FP=V1.V2 ET CFM=1/2(V1*.V2+V1.V2*) Eq 7.16 Xiabo Chen's Thesis
            DO J=1,6
...
!! EFWPS(NO CONTRIB , DOF(+6 POUR PARTIE IMAG) , NO CORPS)
                  EFWPS(1,J,KNC)=EFWPS(1,J,KNC)+FP(1)*ACQ               !! INTEGRATION CORRECTE  (Real part)
                  EFWPS(1,J+6,KNC)=EFWPS(1,J+6,KNC)+FP(2)*ACQ           !! imaginary part
                  EFWMN(1,J,KNC)=EFWMN(1,J,KNC)+FM(1)*ACQ               !! real part
                  EFWMN(1,J+6,KNC)=EFWMN(1,J+6,KNC)+FM(2)*ACQ           !! imaginary part
            END DO
      END DO
      END DO
...
C _________________________________________________
      DO J=1,3
            PRO2(J,1)=FHY1(J)           !Inertia force [Fx,Fy,Fz]
            PRO2(J,2)=FHY1(J+6)
            PRO2(J,3)=FHY2(J)
            PRO2(J,4)=FHY2(J+6)
      END DO
      DO J=1,3
            ...
            CALL PRODT(PRO1,PRO2,FP,FM,3)       ! L x F
      ...
      Q8(2,I1,I2)=EFWMN(8,7,KNC)             ! shouldn't it be 9or[6,12]???
...
!EFFORTS IN RELATION TO THE CALCULATION POINT IN MOTION WITH THE BODY
...
! ################## APROXIMATION DE PINKSTER #########################
      ...
      DO II=IA,IB
            ...
            DO J=1,6!INTEGRATION POUR CHAQUE DOF
                  ...
                  FKR(J)=FKR(J)+ACQ*PHIR/2     !!-\IINT(PHI.N.DS)
      ...
      IF(H.NE.0 .AND.H.LT.0.9E20) THEN ! finite depth
            AK1=AM1*H
            AK2=AM2*H
            AKK=(AK1-AK2)
            AKM=X01(AKK)
      ...
      ENDIF
      WM=SQRT((AM1-AM2)*G*TANH((AM1-AM2)*H))!!!EFFORTS D'EXCITATIONS A LA PULSATION K1-K2  , dispersion
c~       DO IPP=2,NHASKIND                                              ! version matrice carre pour les QTF+   square matrix version for QTF +
      DO IPP=2,N                                                        ! version matrice antitriangulaire sup pour les QTF+ (0 ailleurs)
            IF(WWX(IPP).GT.WM) THEN                                     ! antitriangular matrix version sup for QTF + (0 elsewhere)
                  IPM=IPP
                  GOTO 3886
            ENDIF
      END DO
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      IF (Loutduok==1) THEN
	    OUT1=1	! ON ECRIT TOUTES LES CONTRIBUTIONS    !! WE WRITE ALL CONTRIBUTIONS
      ELSE
	    OUT1=7	! ON ECRIT SUELEMENT DUOK ET PINKSTER  ! ON WRITING ONLY DUOK AND PINKSTER
      ENDIF
      IF (LQTFP .EQ. 1) THEN
            NQTFP=2     ! write the QTF- and QTF+
      ELSE
            NQTFP=1     ! write only QTF-
      ENDIF
      
      DO DOF=1,6,DOFSYM        ! ECRITURE DES RESULTATS
...
c~                 DO I=1,NHASKIND+1                                    ! version matrice carre pour les QTF+
                DO I=1,N+1                                              ! version matrice antitriangulaire sup pour les QTF+ (0 ailleurs)
                  ...
                  ENDDO
                ENDDO
                DO I=2,N+1                                      ! 2 car sinon on change aussi la pulsation  ! 2 because otherwise we also change the pulsation 
                IF (L==2 .AND. K==1) THEN
                     !!!  QTFij-=QTFji-* !!!
...
c~                   WRITE(IUNI,*)(FCONT(DOF,K,I,J,L,M),J=1,NHASKIND+1)         ! version matrice carre pour les QTF+
                  WRITE(IUNI,*)(FCONT(DOF,K,I,J,L,M),J=1,N+1)           ! version matrice antitriangulaire sup pour les QTF+ (0 ailleurs)
                END DO
...
c~             DO I=1,NHASKIND+1                                        ! version matrice carre pour les QTF+
            DO I=1,N+1                                                  ! version matrice antitriangulaire sup pour les QTF+ (0 ailleurs)
                  ...
                  END DO
                  END DO 
c~                   WRITE(IUNI,*) (EFFP(DOF+6*(L-1),I,J,2),J=1,NHASKIND+1)! (2 : # APPROX)     ! version matrice carre pour les QTF+
                  WRITE(IUNI,*) (EFFP(DOF+6*(L-1),I,J,2),J=1,N+1)! (2 : # APPROX)               ! version matrice antitriangulaire sup pour les QTF+ (0 ailleurs)
            END DO
...
C *** WRITE MARK END OF FILE TRANSFER FUNCTION ***     
      ZERO=0.
C BASSE FREQUENCE
      IF(IECRBF.EQ.1) THEN
